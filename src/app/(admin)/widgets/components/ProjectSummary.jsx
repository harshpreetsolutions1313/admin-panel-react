...
const ProjectSummary = () => {
  const projectData = useFetchData(getAllProjects);
  return <>
      <Card>
        <CardBody className="p-0">
          <div className="pt-3 px-3">
            <div className="float-end">
              <Link to="" className="text-primary icons-center">
                Export
                ...
                <tbody>
                  {projectData?.map((item, idx) => <tr key={idx}>
                      ...
                      <td className="avatar-group">
                        {item.teamMembers.map((avatar, idx) => <Link to="" className="avatar-group-item" key={idx}>
                            <img src={avatar} alt="avatar-2" className="img-fluid avatar-xs rounded-circle avatar-border" />
                          </Link>)}
                      </td>
