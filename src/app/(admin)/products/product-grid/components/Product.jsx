...
const ProductCard = ({
  ...
  return <Card>
      <img src={image} alt="productImg" className="img-fluid" />
      <CardBody className="bg-light-subtle rounded-bottom">
        <Link to={`/products/${id}`} className="text-dark fw-medium fs-16">
          {title}
        </Link>
        <div className="my-1">
        ...
        <div className="mt-3">
          <div className="d-flex gap-2">
            ...
            <Link to="/orders/order-cart" className="btn btn-outline-dark border border-secondary-subtle d-flex align-items-center justify-content-center gap-1 w-100">
...
const Product = () => {
  ...
  return <Row>
      {productData?.map((item, idx) => <Col md={6} xl={3} key={idx}>
          <ProductCard {...item} />
        </Col>)}
    </Row>;
};
