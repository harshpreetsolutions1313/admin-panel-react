...
import { Card, CardBody, CardFooter, Col, Dropdown, DropdownMenu, DropdownToggle, ProgressBar, Row } from 'react-bootstrap';
  ...
  return <Card>
      <CardBody>
        <div className="position-relative bg-light p-2 rounded text-center">
          <img src={image} alt="SellersImage" className="avatar-xxl" />
      ...
      <CardFooter className="border-top gap-1 hstack">
        ...
        <Link to="" className="btn btn-soft-danger d-inline-flex align-items-center justify-content-center rounded-circle avatar-sm">
          <span>
...
const SellerListPage = () => {
  const sellersData = useFetchData(getSellersData);
  return <>
      <PageTItle title="Sellers List" />
      <Row>
        {sellersData?.map((item, idx) => <Col xl={3} md={6} key={idx}>
            <SellersCard {...item} />
          </Col>)}
      </Row>
    </>;
